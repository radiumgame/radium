...
import Radium.Engine.PostProcessing.EffectUniform;
import Radium.Engine.PostProcessing.PostProcessingEffect;
import Radium.Engine.PostProcessing.UniformType;
...
public class PostProcessing extends Component {
    ...
    public void Update() {
        effects = Radium.Engine.PostProcessing.PostProcessing.GetEffects();
    ...
    public void GUIRender() {
        ...
        if (ImGui.treeNodeEx("Effects", flags)) {
            for (int i = 0; i < Radium.Engine.PostProcessing.PostProcessing.GetEffects().size(); i++) {
                PostProcessingEffect effect = Radium.Engine.PostProcessing.PostProcessing.GetEffects().get(i);

                if (ImGui.button("Remove##" + i)) {
                    Radium.Engine.PostProcessing.PostProcessing.RemoveEffect(effect);
                }
                ImGui.sameLine();
            ...
            for (int i = 0; i < Radium.Engine.PostProcessing.PostProcessing.customEffects.size(); i++) {
                CustomPostProcessingEffect effect = Radium.Engine.PostProcessing.PostProcessing.customEffects.get(i);

                if (ImGui.button("Remove##" + i)) {
                    Radium.Engine.PostProcessing.PostProcessing.customEffects.remove(effect);
                }
                ImGui.sameLine();
    ...
    private void AddPopup() {
        ...
        if (ImGui.beginPopup("AddEffect")) {
            for (PostProcessingEffect effect : Radium.Engine.PostProcessing.PostProcessing.effectList) {
            ...
            if (ImGui.treeNodeEx("Custom Effect", ImGuiTreeNodeFlags.SpanAvailWidth)) {
                ...
                if (ImGui.isItemClicked(0)) {
                    ...
                    if (FileExplorer.IsPathValid(customFrag)) {
                        Radium.Engine.PostProcessing.PostProcessing.customEffects.add(new CustomPostProcessingEffect(customFrag));
                    }
